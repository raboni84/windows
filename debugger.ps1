...
Get-ChildItem "$ENV:WINDIR" -recurse | where {$_.extension -in ".exe",".dll",".sys"} | % {
  ...
  if ($pdb -And $guid) {
    ...
    if (-Not(Test-Path -Path $locallink -PathType Leaf)) {
      $StatusCode = ""
      try {
        $Response = Invoke-WebRequest -Uri $linktodbg -Method "Head" -UseBasicParsing
        $StatusCode = $Response.StatusCode
      } catch {
        $StatusCode = $_.Exception.Response.StatusCode.value__
      }
      if ($StatusCode -eq 200) {
        Write-Host "  => $($locallink)"
        ...
        Invoke-WebRequest -Uri $linktodbg -OutFile $locallink -UseBasicParsing
